...
function FolderItem({ folder }: { folder: Folder }) {
  ...
  return (
    <button onClick={() => selectFolder(folder.id)}
      ...
      <Icon size={15} className={isActive ? 'text-primary-600 dark:text-primary-400' : 'text-gray-400'} />
  ...
  return (
    <div className="mb-0.5">
      <button onClick={() => setExpanded(!expanded)}
        className="flex w-full items-center gap-1 rounded px-1.5 py-1 text-xs font-semibold text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300">
        {expanded ? <ChevronDown size={13} /> : <ChevronRight size={13} />}
        <Mail size={13} className="text-gray-400" />
        <span className="truncate">{accountName}</span>
      </button>
      {expanded && (
        <div className="ml-2 space-y-px pl-2 border-l border-gray-200 dark:border-gray-700">
