  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? 'bg-gray-200 text-gray-800' : 'text-gray-400 hover:bg-gray-100 hover:text-gray-600',
      )}
    >
      <Icon size={15} />
    </button>
...
export function Composer() {
  ...
  if (minimized) {
    return (
      <div className="fixed bottom-0 right-6 z-50 w-72 rounded-t-lg border border-gray-300 bg-gray-800 shadow-2xl">
        <div
        ...
        >
          ...
          <button onClick={(e) => { e.stopPropagation(); closeComposer(); }} className="text-gray-400 hover:text-white">
            <X size={16} />
          </button>
        </div>
      </div>
  ...
  return (
    <div className="fixed bottom-0 right-6 z-50 flex w-[560px] flex-col rounded-t-xl border border-gray-300 bg-white shadow-2xl" style={{ maxHeight: '80vh' }}>
      <div className="flex items-center gap-2 rounded-t-xl bg-gray-800 px-4 py-2.5">
        ...
        <button onClick={closeComposer} className="text-gray-400 hover:text-white">
          <X size={16} />
        </button>
      </div>
      ...
      <div className="space-y-0 border-b border-gray-100">
        <div className="flex items-center border-b border-gray-50 px-4 py-1.5">
          <label className="w-10 shrink-0 text-xs text-gray-400">To</label>
          <input
            value={to}
          ...
          />
          {!showCcBcc && (
            <button onClick={() => setShowCcBcc(true)} className="text-xs text-gray-400 hover:text-gray-600">
              Cc/Bcc
            </button>
          )}
        </div>
        {showCcBcc && (
          <>
            <div className="flex items-center border-b border-gray-50 px-4 py-1.5">
              <label className="w-10 shrink-0 text-xs text-gray-400">Cc</label>
              <input
                value={cc}
            ...
            <div className="flex items-center border-b border-gray-50 px-4 py-1.5">
              <label className="w-10 shrink-0 text-xs text-gray-400">Bcc</label>
              <input
                value={bcc}
        ...
        <div className="flex items-center px-4 py-1.5">
          <label className="w-10 shrink-0 text-xs text-gray-400">Sub</label>
          <input
            value={subject}
      ...
      <div className="flex-1 overflow-y-auto" style={{ minHeight: 200 }}>
        <EditorContent editor={editor} className="text-sm text-gray-700" />
      </div>

      {editor && (
        <div className="flex items-center gap-0.5 border-t border-gray-100 px-3 py-1.5">
      ...
      <div className="flex items-center gap-2 border-t border-gray-200 px-4 py-2.5">
        ...
        <button onClick={closeComposer} className="rounded-md p-2 text-gray-400 hover:bg-red-50 hover:text-red-500">
          <Trash2 size={16} />
        </button>
      </div>
    </div>
