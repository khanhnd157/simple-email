...
function ContactRow({ contact, selected }: { contact: ContactEntry; selected: boolean }) {
  ...
  return (
    <div
      ...
      className={cn(
        'group flex items-center gap-3 cursor-pointer border-b border-gray-100 px-4 py-2.5 transition-colors',
    ...
    >
      ...
      {contact.tags.length > 0 && (
        <span className="hidden group-hover:hidden lg:inline-flex items-center rounded-full bg-gray-100 px-2 py-0.5 text-[10px] text-gray-500">
          {contact.tags[0]}
        </span>
      )}
      <button
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="mt-3 space-y-2">
            ...
            {contact.company && (
              <div className="flex items-center gap-2 text-sm text-gray-600">
                <Building2 size={15} className="text-gray-400" /> {contact.company}
              </div>
            )}
          </div>
          {contact.tags.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-1.5">
              {contact.tags.map((tag) => (
                <span key={tag} className="flex items-center gap-1 rounded-full bg-gray-100 px-2.5 py-0.5 text-xs text-gray-600">
                  <Tag size={10} /> {tag}
                </span>
              ))}
            </div>
          ...
          <button
            ...
            className="mt-4 flex items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-1.5 text-sm text-gray-600 hover:bg-gray-50"
          >
        ...
        <div className="px-3 py-2 border-b border-gray-100">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-md border border-gray-200 bg-gray-50 py-1.5 pl-8 pr-3 text-sm outline-none focus:border-primary-300"
            />
          </div>
        </div>
      ...
      <div className="flex-1">
        {selected ? (
          ...
          <div className="flex h-full flex-col items-center justify-center text-gray-300">
            ...
            <p className="mt-3 text-sm text-gray-400">Select a contact</p>
          </div>
        )}
      </div>
    </div>
