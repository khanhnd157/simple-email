  ...
  return (
    <div className={cn('relative', className)}>
      <select value={value} onChange={(e) => onChange(e.target.value)}
        className="w-full appearance-none rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-1.5 pr-8 text-sm text-gray-700 dark:text-gray-300 outline-none focus:border-primary-400 focus:ring-1 focus:ring-primary-200">
  ...
  return (
    <div className={cn('flex items-center gap-1.5', className)}>
      <input type="number" value={value} min={min} max={max}
        ...
        className="w-16 rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-2 py-1.5 text-sm text-gray-700 dark:text-gray-300 outline-none focus:border-primary-400 text-center" />
...
export function TextInput({ value, onChange, placeholder, className }: {
  ...
  return (
    <input type="text" value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder}
      className={cn('rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-1.5 text-sm text-gray-700 dark:text-gray-300 outline-none focus:border-primary-400 focus:ring-1 focus:ring-primary-200', className)} />
  );
}
...
export function SettingGroup({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-5">
      <h4 className="mb-2.5 text-xs font-semibold uppercase tracking-wider text-gray-400">{title}</h4>
      <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800/50 divide-y divide-gray-100 dark:divide-gray-700/50">
        {children}
      </div>
    </div>
  );
  ...
  return (
    <label className="flex items-center gap-2.5 py-1.5 cursor-pointer group">
      <div className={cn('h-4 w-4 rounded border flex items-center justify-center shrink-0 transition-colors',
        checked ? 'bg-primary-600 border-primary-600' : 'border-gray-300 dark:border-gray-600 group-hover:border-gray-400')}>
      ...
      {options.map((o) => (
        <label key={o.value} className="flex items-center gap-2.5 cursor-pointer group">
          <div className={cn('h-4 w-4 rounded-full border flex items-center justify-center shrink-0',
            value === o.value ? 'border-primary-600' : 'border-gray-300 dark:border-gray-600 group-hover:border-gray-400')}>
...
export function Button({ children, variant = 'secondary', onClick, className }: {
  ...
  return (
    <button onClick={onClick} className={cn(
      ...
      variant === 'secondary' && 'border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800',
